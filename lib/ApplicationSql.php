...
class ApplicationSql {
  ...
  private static function var_to_symbol($_var, $command = "") {
    ...
    if ($_var) {
      ...
      return array("$command $symbol", $symbol, $_var);
    } else {
      return array("", "", "");
    }
  ...
  public static function update($_table, $_sets, $_where) {
    ...
    list($where_key_and_symbols, $where_symbol_and_values) = static::hash_to_keysymbol_symbolvalue($_where, ",", "WHERE");

    $query = $GLOBALS['db']->prepare("UPDATE `$_table` SET $set_key_and_symbols $where_key_and_symbols");
  ...
  public static function delete($_table, $_fields, $_limit) {
    ...
    list($limit_command_symbol,  $limit_symbol,  $limit_value)  = static::var_to_symbol($_limit, "LIMIT");
  ...
  public static function query($_select, $_table, $_join, $_where, $_order, $_group, $_limit, $_offset) {
    ...
    list($limit_command_symbol,  $limit_symbol,  $limit_value)  = static::var_to_symbol($_limit, "LIMIT");
    list($offset_command_symbol, $offset_symbol, $offset_value) = static::var_to_symbol($_offset, "OFFSET");
